...
def windowsetup():
    ...
    close_gui=0
    window=tk.Tk()
    window.title("UAV Controls")
    window.geometry('1100x400')

    heading_text = tk.Label(window,text="Heading:")
    heading_text.grid(column=1,row=1)
    heading_gui_data=tk.IntVar()
#   heading_gui_data.set(heading)
    heading_gui = tk.Label(window,textvariable=heading_gui_data)
    heading_gui.grid(column=2,row=1)

    roll_text = tk.Label(window,text="Roll:")
    roll_text.grid(column=1,row=2)
    roll_gui_data=tk.IntVar()
#   roll_gui_data.set(roll)
    roll_gui = tk.Label(window,textvariable=roll_gui_data)
    roll_gui.grid(column=2,row=2)

    pitch_text = tk.Label(window,text="Pitch:")
    pitch_text.grid(column=1,row=3)
    pitch_gui_data=tk.IntVar()
    pitch_gui = tk.Label(window,textvariable=pitch_gui_data)
    pitch_gui.grid(column=2,row=3)

    depth_text = tk.Label(window,text="Depth:")
    depth_text.grid(column=1,row=4)
    depth_gui_data=tk.IntVar()
    depth_gui = tk.Label(window,textvariable=depth_gui_data)
    depth_gui.grid(column=2,row=4)

    port_pw_text=tk.Label(window,text="Port pw:")
    port_pw_text.grid(column=1,row=5)
    port_gui_data=tk.IntVar()
    port_gui = tk.Label(window,textvariable=port_gui_data)
    port_gui.grid(column=2,row=5)

    star_pw_text=tk.Label(window,text="Stbd pw:")
    star_pw_text.grid(column=1,row=6)
    star_gui_data=tk.IntVar()
    star_gui=tk.Label(window,textvariable=star_gui_data)
    star_gui.grid(column=2,row=6)

    fore_pw_text=tk.Label(window,text="Fore pw:")
    fore_pw_text.grid(column=1,row=7)
    fore_gui_data=tk.IntVar()
    fore_gui=tk.Label(window,textvariable=fore_gui_data)
    fore_gui.grid(column=2,row=7)

    aft_pw_text=tk.Label(window,text="Aft pw:")
    aft_pw_text.grid(column=1,row=8)
    aft_gui_data=tk.IntVar()
    aft_gui=tk.Label(window,textvariable=aft_gui_data)
    aft_gui.grid(column=2,row=8)

    heading_field_text=tk.Label(window,text="Enter heading:")
    heading_field_text.grid(column=3,row=1)
    heading_field=tk.Entry(window)
    heading_field.grid(column=4,row=1)
    heading_field_button = tk.Button(window,text="Go to heading", command=lambda: check_gui_heading(heading_field.get()))
    heading_field_button.grid(column=5,row=1)
    heading_field_stop = tk.Button(window,text="Stop heading", command=lambda: stop_persistent_h.set())
    heading_field_stop.grid(column=6,row=1)

    depth_field_text=tk.Label(window,text="Enter depth:")
    depth_field_text.grid(column=3,row=4)
    depth_field=tk.Entry(window)
    depth_field.grid(column=4,row=4)
    depth_set_indicator=tk.Label(window,text="Depth setup",bg="white")
    depth_set_indicator.grid(column=5,row=3)
    depth_field_button = tk.Button(window,text="Go to depth",command=lambda: check_gui_depth(depth_field.get()))
    depth_field_button.grid(column=5,row=4)
    depth_zero_button = tk.Button(window,text="Depth zero",command=lambda: depth_zero())
    depth_zero_button.grid(column=5,row=5)
    depth_set_button = tk.Button(window,text="Depth set",command=lambda: depth_set())
    depth_set_button.grid(column=5,row=6)

    Horizon_button = tk.Button(window,text="EMERGENCY STOP",command=lambda: horizon())
    Horizon_button.grid(column=2,row=0)

    command_indicator = tk.Label(window,text="Command Function",bg="white")
    command_indicator.grid(row=0,column=6)
    call_indicator = tk.Label(window,text="Call Function",bg="white")
    call_indicator.grid(row=0,column=7)

    Test1_button = tk.Button(window,text="Test fwd/back",command=lambda: direction_command(1))
    Test1_button.grid(column=8,row=1)
    Test2_button = tk.Button(window,text="Test left/right",command=lambda: direction_command(2))
    Test2_button.grid(column=8,row=2)
    Test3_button = tk.Button(window,text="Test up/down",command=lambda: direction_command(3))
    Test3_button.grid(column=8,row=3)
    Test4_button = tk.Button(window,text="Test pitch",command=lambda: direction_command(4))
    Test4_button.grid(column=8,row=4)

    Quit_button=tk.Button(window,text="Leave GUI",command=lambda: close_gui_set())
    Quit_button.grid(column=8,row=0)
    
    reset_button=tk.Button(window,text="Reset Mbed",command=lambda: reset_mbed())
    reset_button.grid(column=0,row=0)
    
    while not stop_threads.is_set():
    #while not close_gui.is_set():
    #    if stop_threads.is_set():
    #        close_gui.set()
        heading_gui_data.set(heading)
        roll_gui_data.set(roll)
        pitch_gui_data.set(pitch)
        depth_gui_data.set(depth)
        port_gui_data.set(port_pw)
        star_gui_data.set(starboard_pw)
        fore_gui_data.set(fore_pw)
        aft_gui_data.set(aft_pw)
        if ((status&0x0008)==0x0008):
            depth_set_indicator.config(bg="green2")
        else:
            depth_set_indicator.config(bg="white")
        if ((status&0x0100)==0x0100):
            command_indicator.config(bg="green2")
        else:
            command_indicator.config(bg="white")
        if ((status&0x0200)==0x0200):
            call_indicator.config(bg="green2")
        else:
            call_indicator.config(bg="white")       
        window.update()
...
def main():
    ...
    while (quit!=1):
        print()
        print("Mode select: [c]onfig, [t]est mode, [m]onitor data, [d]epth, [g]raphic interface, ")
        print("[h]eading, [p]ersistent heading, [r]andom heading, [q]uit")
