...
volatile float heading_Pk =4.0f;        //approx +8.9% for every 20deg
volatile float heading_Ik =10.0f;       //doubles response approx every x seconds
volatile float heading_Dk =0.0f;
...
volatile float depth_Pk =6.0f;
volatile float depth_Ik =10.0f;
volatile float depth_Dk =0.0;
PID pid_depth(depth_Pk, depth_Ik, depth_Dk, ticker_rate);

volatile float pitch_Pk = 6.0f; //1.0f;
volatile float pitch_Ik = 10.0f; //10.0f;
volatile float pitch_Dk = 0.00001;  //0.0;
...
double az_tolerance=2; //deg of acceptable heading tolerance
...
double pitchController(void){
    ...
    if (desired_pitch!=-1) {
        ...
        //if (desired_pitch>current_pitch) dir=(-1*dir);
        //diff = abs(desired_pitch-current_pitch);
        ...
        */
        //Zero out integral PID gain when pitch is within tolerance.
        /*
        ...
        */
            ...
            if ((fabs(speed)<min_thruster_bias) and (diff!=0)) {
                if (speed<0) speed=(min_thruster_bias);
...
double depthController(void){
    ...
    double dir=1;
    double factor;
    //Zero out integral PID gain when depth is within tolerance.
