...
def video_overlay():
    ...
    json_data={'ser_h':str(h), 'ser_r':str(r), 'ser_p':str(p), 'ser_d':str(d), 'ser_port':p_pw, 'ser_stbd':s_pw, 'ser_fwd':f_pw, 'ser_aft':a_pw, 'ser_cal':cal_data, 'ser_status':status_data}
    serial_data_file = open(serial_data_directory+"serial_JSON", 'w')
    json.dump(json_data,serial_data_file)
    serial_data_file.close()

def shutdownPi():
    log.info("Water leak detected. All systems shut down.")
