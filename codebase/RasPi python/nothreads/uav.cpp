...
/*
double calculate_az_curve_values() {
	double log_val=((esc_range_scale/min_thruster_bias)-1);
	double natlog=log(log_val);
	az_center=(natlog/az_slope)+az_tolerance;
}
*/
...
/*
    ...
    if (_lock==1) {
        ...
        while(diff_pw>tolerance_pw) {
            ...
            if (_lock==0) {
                pc.printf("log: pwm pin int: %i, char: %c",_pin, _pin);
                set_pw(new_pw);
            }
            wait_ms(dwell_time);
...
//    double press_slope=(cal_depth-zero_depth)/depth_setting;
    ...
    double depth=press_sensor.depth();
...
void Stbd_thread(double *d_pntr) {
    pc.printf("log: THREAD START, stbd\r\n");
    double pw=*d_pntr;
    starboard_thrust.set_speed(pw);
    pc.printf("log: THREAD END, stbd\r\n");
}
void Port_thread(double *d_pntr) {
    pc.printf("log: THREAD START, port\r\n");
    double pw=*d_pntr;
    port_thrust.set_speed(pw);
    pc.printf("log: THREAD END, port\r\n");
}
void Fwd_thread (double *d_pntr) {
    pc.printf("log: THREAD START, fwd\r\n");
    double pw=*d_pntr;
    fore_thrust.set_speed(pw);
    pc.printf("log: THREAD END, fwd\r\n");
}
void Aft_thread(double *d_pntr) {
    pc.printf("log: THREAD START, aft\r\n");
    double pw=*d_pntr;
    aft_thrust.set_speed(pw);
    pc.printf("log: THREAD END, aft\r\n");
}
...
int call_threads(int select, double dir=1, double speed=0) {
    ...
    if (call_threads_available==1) {
        ...
        ready_data=((ready_data & 0xfdff) | 0x0200);
...
int wait_logic_available() {
    while (logic_available!=1) {
        wait_ms(1);
    }
    return 1;
}
...
void el_thruster_logic(int desired_depth=persistent_depth) {
...
//    double current_depth=(((el_data(0x1234abcd))/resolution)*cm_conversion);    //cm
    ...
    double pw_tolerance=0.015;
    double current_pw=aft_thrust.get_pw();
    double compare;
...
//        int check_threads=0;
//        check_threads=
        compare=fabs((1.5+(dir*speed))-current_pw);
        if (compare>pw_tolerance) {
        ...
        if ((event_horizon_flag!=0) or (persistent_depth==-1)) diff=tolerance;
    }
    if ( abs(current_pw-1.5)>.001) {
        if (call_threads_available==1) call_threads(99);
    }
}

//Function to handle lateral motor logic.
void az_thruster_logic(int desired_heading=persistent_heading) {
    ...
    double pw_tolerance=0.015;
    double current_pw=starboard_thrust.get_pw();
    double compare;
    //Sigmoid data
    ...
    if (desired_heading>=0 and desired_heading<=360) {
        ...
        while ((diff > tolerance) and (event_horizon_flag==0)) {
            dir=1;
            speed=0;
            az_data();
            ...
            speed=Sigmoid(esc_range,slope,center,diff);
            if (speed<0.03) speed=0.03;
            current_pw=starboard_thrust.get_pw();
...
//            int check_threads=0;
            ...
            if (check_thrusters==1) pc.printf("Threads rejoined, heading: %f, count: %i\r\n",current_heading, i);
            wait_ms(2.5);
            //if (event_horizon_flag!=0) diff=tolerance;
        }
        if ( abs(current_pw-1.5)>.001) {
            if (call_threads_available==1) call_threads(0);
        }
...
void commands() {
    command_available=0;
    ready_data=((ready_data&0xfeff) | 0x0100);
    ...
    //    if (i==7) {
            ...
            if (verified_hea==3) {
                ...
                if (hea==999) persistent_heading=-1;
                pc.printf("log: heading rx: %i\r\n",hea);
                //wait_logic_available();
                logic_available=0;
            ...
            if (verified_dep==3) {
                ...
                if (dep==999) persistent_depth=-1;
                else {
                    //wait_logic_available();
                    logic_available=0;
                    el_thruster_logic(dep);
                    logic_available=1;
                    persistent_depth=dep;
                    if (wait_call_threads_available()==1) call_threads(99);
                }
