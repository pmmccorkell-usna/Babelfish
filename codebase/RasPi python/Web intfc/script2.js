...
function send_K(prefix,new_K) {
  var pow10 = [100000.0, 10000.0, 1000.0, 100.0, 10.0, 1.0, 0.1, 0.01, 0.001, 0.0001];
  var sigdigits, power, send_value;
  power = Math.floor(Math.log10(new_K))+4);
  sigdigits = new_K*pow10[power];
  send_value = sigdigits.toString()+power.toString();
  send_cmd2(prefix+send_value);
}
function stop_all() {
      prev_pitch=900;
      prev_depth=900;
