...
int press_sensor_type=1;
int sensor_rate=512;         //Oversampling Rate, see data sheet
float depth=0;     //cm
float depth_tolerance=0.1;   //cm of acceptable depth tolerance
...
volatile float ticker_rate= 0.1;
volatile float heading_Pk =2.0f;        //approx +8.9% for every 20deg
volatile float heading_Ik =2.0f;       //doubles response approx every x seconds
...
void increment_persistent(int select) {
	int pitch_resolution=3;		//degrees
	int depth_resolution=2;		//cm
	int heading_resolution=3;	//degrees
	int speed_resolution=10;	//us
	switch (select) {
		//pitch
		case 10:
			persistent_pitch=(pitch/16)-pitch_resolution;
			pid_pitch.reset();
			break;
		case 11:
			persistent_pitch=(pitch/16)+pitch_resolution;
		...
		//depth
		case 20:
			persistent_depth=depth-depth_resolution;
			pid_depth.reset();
			break;
		case 21:
			persistent_depth=depth+depth_resolution;
		...
		//heading
		case 30:
			persistent_heading=(heading/16)-heading_resolution;
			pid_heading.reset();
			break;
		case 31:
			persistent_heading=(heading/16)+heading_resolution;
		...
		//speed
		case 40:
			persistent_speed=(1000*(starboard_thrust.get_speed())-speed_resolution;
			pid_speed.reset();
			break;
		case 41:
			persistent_speed=(1000*(starboard_thrust.get_speed())+speed_resolution;
...
void commands() {
    ...
    //    if (i==7) {
		...
		if (verified_hea==3) {
			...
			if (hea==844) {
				increment_persistent(30);
			}
			if (hea=877) {
				increment_persistent(31);
			}
		...
		if (verified_dep==3) {
			...
			if (dep>=997) {
				...
				else if (dep==997) increment_persistent(20);
				else if (dep==998) increment_persistent(21);
			}
			if 
			else {
				pid_depth.reset();
		...
		if (verified_zer==3) {
			...
			zero_set=1;
			pc.printf("log: zeroized at %f Pa\r\n",zero);
		}
		if (verified_pit==3) {
			//Correct for ascii '0', and reform 3digit decimal number
			...
			if (pit==844) {
				increment_persistent(10);
			}
			if (pit==877) {
				increment_persistent(11);
			}
		...
		if (verified_vel==3) {
			...
			if (vel==844) {
				increment_persistent(40);
			}
			if (vel==877) {
				increment_persistent(41);
			}
			if ((vel>=0) and (vel<=800)) {
				pid_pitch.reset();
				if (event_horizon_flag==0) persistent_pitch=vel;
			}
		}
    }
}
