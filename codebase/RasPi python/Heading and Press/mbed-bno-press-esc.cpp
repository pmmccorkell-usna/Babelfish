...
void az_thruster_logic(int desired_heading) {
    ...
    //The natural #
    pc.printf("Az Thruster Logic called\r\n");
...
void manual_direction(int direction,int mode) {
    ...
    if (mode==0) {
        ...
        if (call_threads_available==1) call_threads(mode,dir,speed);
    }
}
...
void EventHorizon() {
    horizon_count++;
    pc.printf("EventHorizon called, count: %i\r\n",horizon_count);
...
void commands() {
    ...
    if (length==7) {
        ...
        pc.printf("Command thread called\r\n");
        char input[10];
        for (int i=0; i<10; i++) {
            input[i]=readline[i];
